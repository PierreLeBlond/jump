...
var input_source: INPUT_SOURCE = INPUT_SOURCE.KEYBOARD
...
func _ready():
    line_edit = score_submit.player_name_input

    line_edit.focus_exited.connect(hide)
    line_edit.focus_entered.connect(show)
...
func _input(event):
    ...
    if event.button_index == JOY_BUTTON_DPAD_LEFT:
        update_current_button(get_left_button(current_button))
    elif event.button_index == JOY_BUTTON_DPAD_DOWN:
        update_current_button(get_down_button(current_button))
    elif event.button_index == JOY_BUTTON_DPAD_UP:
        update_current_button(get_up_button(current_button))
    elif event.button_index == JOY_BUTTON_DPAD_RIGHT:
        update_current_button(get_right_button(current_button))
    elif event.button_index == JOY_BUTTON_A:
        current_button._on_button_down()
        current_button._on_button_up()
    elif event.button_index == JOY_BUTTON_B:
        _key_released({"output": "Tab"})
...
func _switch_layout(key_data):
    ...
    for layout in layouts:
        if layout.get_meta("layout_name") == key_data.get("layout-name"):
            _show_layout(layout)
            update_current_button(switch_layout_keys[key_data.get("layout-name")])
            return
...
func _key_released(key_data):
    if key_data.has("output"):
        ...
        input_event_key.unicode = key if key_value != "Tab" else 0 # Fixes tab key being interpreted as a character
            ...
            for key in row.get("keys"):
                ...
                if key.has("type"):
                    if key.get("type") == "switch-layout":
                        ...
                        switch_layout_keys[layout_name] = new_key
                        ...
                        new_key.released.connect(func(key_data): update_current_button(key_data.button))
                else:
                    new_key.released.connect(func(key_data): update_current_button(key_data.button))
...
func update_current_button(new_current_button: Button):
    if !new_current_button || new_current_button == current_button:
        return

    if current_button:
        current_button.button_pressed = false

    new_current_button.button_pressed = true
    current_button = new_current_button

func get_left_button(button: Button):
    var parent = button.get_parent()
    var index = (parent.get_children().find(button) - 1) % parent.get_children().size()
    return parent.get_children()[index]

func get_right_button(button: Button):
    var parent = button.get_parent()
    var index = (parent.get_children().find(button) + 1) % parent.get_children().size()
    return parent.get_children()[index]

func get_down_button(button: Button):
    var parent = button.get_parent().get_parent()
    var index = (parent.get_children().find(button.get_parent()) + 1) % parent.get_children().size()
    var line = parent.get_children()[index]
    var down_button = line.get_children()[0]
    for candidate_button in line.get_children():
        if abs(button.global_position.x - candidate_button.global_position.x) < abs(button.global_position.x - down_button.global_position.x):
            down_button = candidate_button
    return down_button

func get_up_button(button: Button):
    var parent = button.get_parent().get_parent()
    var index = (parent.get_children().find(button.get_parent()) - 1) % parent.get_children().size()
    var line = parent.get_children()[index]
    var up_button = line.get_children()[0]
    for candidate_button in line.get_children():
        if abs(button.global_position.x - candidate_button.global_position.x) < abs(button.global_position.x - up_button.global_position.x):
            up_button = candidate_button
    return up_button
